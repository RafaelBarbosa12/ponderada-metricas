...
| Este índice de entrega | https://github.com/RafaelBarbosa12/ponderada-metricas/blob/main/entregaveis/ENTREGAVEIS.md |
...
**Histórico completo (22 runs, 12+ variações):** coletado no repositório anterior `ponderada-hermano` — dados preservados em `data/metrics/metrics_latest.csv` e analisados em `docs/RELATORIO.md` (seção 4 e gráficos).

**Repositório atual (`ponderada-metricas`):** contém o código final e a run **#1** documentada acima. Para repetir as 12 variações só neste repo:
...
Respostas detalhadas (etapas lentas, cache, paralelismo, falhas, melhorias, limitações, decisões de engenharia, hipóteses, resultados inesperados):
...
> Índice de entregáveis: https://github.com/RafaelBarbosa12/ponderada-metricas/blob/main/entregaveis/ENTREGAVEIS.md  
...
Código e experimento inicial: https://github.com/RafaelBarbosa12/ponderada-hermano  
Substituído por `ponderada-metricas` na entrega final por histórico confuso de falhas de CI no Actions.
